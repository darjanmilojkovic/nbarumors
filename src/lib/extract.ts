...
export type Extraction = {
  ...
  isRoundup: boolean;
  /** Optional so hand-built extractions (backfill scripts) need not set it. */
  isTradeIdea?: boolean;
  headline: string;
  body: string;
...
export const SCHEMA = {
  ...
  properties: {
    ...
    isTradeIdea: {
      type: "boolean",
      description:
        "True when the move exists only because someone PROPOSED it: a writer's or analyst's trade idea, a trade pitch, a mock trade, a podcast suggestion, a 'five trades that would help X' list, an opinion that a player would be a good fit or should be a target. The test is whether anyone with knowledge of a team's thinking has REPORTED it. 'Four-team idea sends Derrick Jones Jr. to the Lakers for Knecht', 'Pincus names Randle as the Lakers' top trade target' and 'Duren to Sacramento for Sabonis and three firsts pitched' are trade ideas: nobody reported the teams discussing them. False for reported interest, talks, offers, negotiations or deals, however thinly sourced: 'the Knicks have interest in Bruce Brown, per sources' and 'Hawks and Hornets discussing a Hield swap' are reports, not ideas. False for a player or executive speaking on the record about his own situation. A roundup can be a trade idea when every part of it is a proposal.",
    },    headline: {
      type: "string",
      description:
...
/**
 * Whether a summary talks about the article, or disclaims it, instead of
 * saying what it reports: "The piece argues...", "None of this reflects an
 * actual offer, just analysis". Both were already against the prompt and
 * still reached 6 of 258 posts in the fortnight to 26 Sep 2026.
 *
 * "the report" and "the story" are left out on purpose: "the report says the
 * deal is fully guaranteed" is ordinary attribution. "the post" is left out
 * because it is a place on a basketball court.
 */
const META = [
  /*
   * Only with a verb of saying after it. In trade talk "the piece" is also an
   * asset — "a second-round pick is suggested as the piece going back" — and
   * that sentence is fine.
   */
  /\b(the|this) (piece|article|column|write-?up)( itself| also)? (argues|notes|suggests|says|raises|covers|lists|runs|has|calls|is framed|frames|claims|floats|points|mentions|adds|proposes|contends|predicts)\b/i,
  /\b(cited|reported|mentioned) in the (piece|article|column)\b/i,
  /\bthe (piece|article|column) is (analysis|speculation|framed)\b/i,
  // "None of this reflects...", "None of those scenarios are reported as..."
  /\bnone of (this|it|these|those)( \w+)? (reflects?|represents?|is|are)\b/i,
  /\bjust (analysis|possibilities|speculation)\b/i,
...
export async function extractRumor(item: {
    ...
    messages: [
      {
        ...
        content: [
          ...
          ...(retry === "meta"
            ? [
                ``,
                `Your previous summary described the article instead of what it reports ("The piece argues", "The article suggests", "It also floats"), or ended with a disclaimer ("None of this reflects an actual offer", "This is speculation"). Write the summary again stating the claims themselves, carried by conditional verbs where nothing has happened: "The Warriors are unlikely to trade before the season but could be active at the February deadline." No sentence about the article, and no disclaimer.`,
              ]
            : []),
          ...(retry === "verb"
            ? [
  ...
  /*
   * The same repair for a summary that reviews the article or disclaims it
   * rather than reporting it. After the outlet check, which is the worse
   * fault; one retry per item, and a second miss stands.
   */
  if (!retry && parsed.isRumor && parsed.body && talksAboutTheItem(parsed.body)) {
    return extractRumor(item, { retry: "meta", onUsage });
  }
